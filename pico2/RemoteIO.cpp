...
void RemoteIO::syncStatus()
{
    ...
    if(syncInputs)
    {
        // TODO: this is quite a large command...
        command[0] = uint8_t(RemoteIOCommand::GetInputs);
        ...
        spi_read_blocking(spi, 0xFF, inputData, 12);
        needInputSync = true;
    }
}

void RemoteIO::syncInputs()
{
    if(!needInputSync)
        return;
    ...
    auto keys = inputData;
    auto mods = inputData[6];

    // keyboard
    // a lot of this is copied from the HID code...
    // check for new keys down
    for(int i = 0; i < 6 && keys[i]; i++)
    {
        auto key = keys[i];
        bool found = false;
    ...
    for(int i = 0; i < 6 && lastKeys[i]; i++)
    {
        auto key = lastKeys[i];
        bool found = false;
        for(int j = 0; j < 6 && keys[j] && !found; j++)
            found = keys[j] == key;
        ...
        if(mapped != ATScancode::Invalid)
            chipset.sendKey(mapped, false);
    }

    // ...and mods
    auto changedMods = lastKeyMod ^ mods;
    auto pressedMods = changedMods & mods;
    ...
    for(int i = 0; i < 8; i++)
    {
        ...
        if(pressedMods & (1 << i))
            chipset.sendKey(mod, true);
        else if(releasedMods & (1 << i))
            chipset.sendKey(mod, false);
    }

    // mouse
    int16_t mouseX = inputData[7] | inputData[ 8] << 8;
    int16_t mouseY = inputData[9] | inputData[10] << 8;
    auto mouseButtons = inputData[11];

    chipset.addMouseMotion(mouseX, mouseY);
    chipset.setMouseButton(0, mouseButtons & 1);
    chipset.setMouseButton(1, mouseButtons & 2);
    chipset.syncMouse();
...
void RemoteIOHost::update()
{
    if(spi_is_readable(spi))
    {
        ...
        switch(static_cast<RemoteIOCommand>(b))
        {
            ...
            case RemoteIOCommand::GetInputs:
            {
                uint8_t buf[13];
    ...
    auto &chipset = sys.getChipset();
    if(chipset.getPICInputs() != lastPICInputs || chipset.getDMARequests() != lastDMARequests || needInputSync)
    {
        setStatusPin(true);
...
void RemoteIOHost::setKeyboardState(const uint8_t keys[6], uint8_t mods)
{
    if(mods == rawKeyboard[6] && memcmp(keys, rawKeyboard, 6) == 0)
        return;

    memcpy(rawKeyboard, keys, 6);
    rawKeyboard[6] = mods;
    needInputSync = true;
}

void RemoteIOHost::updateMouseState(int x, int y, uint8_t buttons)
{
    if(x == 0 && y == 0 && buttons == mouseButtons)
        return;
